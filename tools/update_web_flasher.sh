...
if [ "$#" -eq 1 ]; then
    dst="$ROOT/web-flasher/firmware/codemeter_2432s028r.factory.bin"
    mkdir -p "$(dirname "$dst")"
    cp "$1" "$dst"
    echo "Updated: $dst"
else
    copy_image cyd_2432s028r codemeter_2432s028r.factory.bin
    copy_image cyd_2432s028r_landscape codemeter_2432s028r_landscape.factory.bin
fi
