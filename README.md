...
설치 후 `BOOT` 버튼을 누르면 `Usage` 화면, `Bluetooth` 화면, `Settings` 화면
순서로 이동합니다. `Settings` 화면에서 표시 카드를 터치하면 `Used`와 `Usage`
표시 기준이 전환되고, 설정은 보드 안에 저장됩니다. `Used`는 사용한 비율,
`Usage`는 남은 사용 가능 비율입니다.
